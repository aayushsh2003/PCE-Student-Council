...
import { Facebook, Instagram, Twitter, Calendar, Download, ExternalLink, Users, Trophy, Camera } from 'lucide-react'
...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  // Fetch club data based on params.id
  const club = clubs.find(c => c.id === params.id)

  return {
    title: `${club?.name} | College Student Council`,
    description: club?.description,
    openGraph: {
      title: `${club?.name} | College Student Council`,
      description: club?.description,
      images: [club?.logo || '/placeholder.svg'],
    },
  }
}
...
export default function ClubPage({ club }: ClubPageProps) {
  ...
  const structuredData = {
    ...
    name: club.name,
    description: club.description,
    url: `https://yourwebsite.com/clubs/${club.id}`,
    logo: club.logo,
    sameAs: [
          ...
          >
            ...
            <div className="p-8">
                ...
                <nav className="flex space-x-4">
                  {club.socialMedia.facebook && (
                    <Link href={club.socialMedia.facebook} target="_blank" rel="noopener noreferrer">
                      <Facebook className="text-blue-600 hover:text-blue-800 transition-colors" aria-label="Facebook" />
                    </Link>
                  )}
                  {club.socialMedia.instagram && (
                    <Link href={club.socialMedia.instagram} target="_blank" rel="noopener noreferrer">
                      <Instagram className="text-pink-600 hover:text-pink-800 transition-colors" aria-label="Instagram" />
                    </Link>
                  )}
                  {club.socialMedia.twitter && (
                    <Link href={club.socialMedia.twitter} target="_blank" rel="noopener noreferrer">
                      <Twitter className="text-blue-400 hover:text-blue-600 transition-colors" aria-label="Twitter" />
                    </Link>
                  )}
                </nav>
              </header>
                        ...
                        {club.gallery.map((image, index) => (
                          <div key={index} className="relative h-40 rounded-lg overflow-hidden">
                            <Image 
                              ...
                              alt={image.alt || `${club.name} gallery image ${index + 1}`} 
                              layout="fill" 
                              objectFit="cover" 
              ...
              <section className="mt-8">
                <h2 className="text-2xl font-semibold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-600">Resources</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <section className="mt-8">
                <h2 className="text-2xl font-semibold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-purple-600 to-pink-600">Contact Us</h2>
                <Card>
                  <CardContent className="p-4">
